...
def run(lr, optimizer, seed=0, epochs = int(5e3), dim = 3, save_path = None,
        save=False):
    if save is True:
        save_path = pathlib.Path(save_path)
        save_path = save_path / f"{optimizer}_optimizer" / f"lr_{lr}" / f"{seed}_seed"
...
if __name__ == '__main__':
    ...
    lrs = [1e-1, 5e-2, 1e-2, 5e-3, 1e-3, 5e-4, 1e-4, 1e-5]
    seeds = [0, 1, 2, 3]
    ESS_history = np.zeros((len(optimizers), len(lrs), len(seeds)))
    start_time = time.time()
    current_time = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    save_path = "Experiment_Results/100_batch_size_experiment" + f"/{current_time}"
    for i, optimizer in enumerate(optimizers):
        for j, lr in enumerate(lrs):
            for k, seed in enumerate(seeds):
                try:
                    ESS = run(lr, optimizer, seed, save=True,
                              save_path=save_path)
                    ESS_history[i, j, k] = ESS
                except:
                    print(f"failure with optimizer={optimizer}, lr={lr}")
    run_time = time.time() - start_time
    print(f"runtime was {run_time/60} min")

    plt.figure()
    plt.scatter(np.array([lrs]).repeat(len(seeds), axis=0), ESS_history[0, :, :], label=optimizers[0])
    plt.scatter(np.array([lrs]).repeat(len(seeds), axis=0), ESS_history[1, :, :], label=optimizers[1], marker="x")
    plt.xscale("log")
    plt.legend()
