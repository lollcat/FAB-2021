...
def run(k_option, loss_type, lr, optimizer="Adamax", seed=0, epochs = int(1e3), dim = 6, save_path = None,
        save=False):
    if save is True:
        save_path = pathlib.Path(save_path)
        save_path = save_path / f"{loss_type}_loss_type" / f"lr_{lr}_k_{k_option}" / f"{seed}_seed"
...
if __name__ == '__main__':
    ...
    import numpy as np
    import time
    loss_types = ["DReG_kl", "DReG"]
    k_options = [1, 10, 100, 1000]
    lrs = [1e-2, 1e-3, 1e-4]
    seeds = [0, 1, 2, 3]
    ESS_history = np.zeros((len(loss_types), len(lrs), len(seeds)))
    start_time = time.time()
    current_time = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    save_path = "Experiment_Results/k_experiment" + f"/{current_time}"
    for k, seed in enumerate(seeds):
        for j, lr in enumerate(lrs):
            for k_option in k_options:
                for i, loss_type in enumerate(loss_types):
                    try:
                        ESS = run(k_option, loss_type, lr, seed=seed, save=True,
                                  save_path=save_path)
                        #ESS_history[i, j, k] = ESS
                    except:
                        print(f"failure with loss_type={loss_type}, lr={lr}, k={k}")
